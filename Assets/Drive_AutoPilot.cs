...
public class Drive_AutoPilot : MonoBehaviour {
    // Tank speed
    ...
    // 回転角度を算出し、タンクを回転させていくメソッド
    // 非数(英語：Not a Number)とは未定義な値か、特に浮動小数点数の計算の結果で、表すことができない数値(データ型)のこと
    void CalculateAngle() {
        ...
        this.transform.Rotate(0.0f, 0.0f, unityAngle * 0.02f);
    }
    ...
    void Update() {
        ...
        if (autoPilot) {
            ...
            // Rotation の Y軸座標 を 0 にしないと distance の計算が正しく行われないことに注意
            if (CalculateDistance() > 8.0f) {
                AutoPilot();
            }
        }
